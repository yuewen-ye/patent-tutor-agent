...
| `retrieval_context` | array[`RetrievalChunk`] | 否 | `retrieve_context`、`expert_a`、`expert_b` | `expert_a`、`expert_b`、`judge`、`chat_answer` |
...
读取：chat 路径为 `user_input`；teach 路径由专家根据 `user_input`、`learning_path`、辩论上下文自行组织检索查询。
...
**各 Agent 独立 RAG 检索**：当前 teach 路径已由 `expert_a` / `expert_b` 按职责自行决定是否调用 RAG；`retrieve_context` 仅保留为 chat 路径固定检索节点。后续 P0.5 可继续扩展到 diagnosis、planner、judge、feedback：
...
```text
planner → expert_a ∥ expert_b  when intent=teach
```
...
| RAG 调用 | teach 路径由 `expert_a` / `expert_b` 自主工具检索；chat 路径由 `retrieve_context` 固定检索 | 继续扩展到 diagnosis/planner/judge/feedback | P0.5 |
...
每个 Agent 节点的完整系统提示词独立维护在节点目录下，代码通过 `backend/app/agents/common.py::load_prompt()` 加载。单阶段 Agent 使用 `system.md`；多阶段 Agent 必须为每个阶段提供 `<阶段名>_system.md`，并由节点代码显式加载。

| Agent | 提示词文件 | 内容来源 |
|-------|-----------|---------|
| `diagnosis` | `backend/app/agents/diagnosis/diagnosis_system.md` | `docs/agents_analysis/01` SOUL + ABILITIES |
| `planner` | `backend/app/agents/planner/system.md` | `docs/agents_analysis/02` SOUL + ABILITIES |
| `expert_a` debate | `backend/app/agents/expert_a/debate_system.md` | `docs/agents_analysis/03` SOUL + ABILITIES |
| `expert_a` integration | `backend/app/agents/expert_a/integration_system.md` | `docs/agents_analysis/03` SOUL + 整合阶段职责 |
...
| `feedback` | `backend/app/agents/diagnosis/feedback_system.md` | `docs/agents_analysis/01` SOUL（反馈分析阶段） |
...
这些文件是运行时提示词来源。Agent 行为需要调整时，先修改对应 `system.md` 或 `<阶段名>_system.md`，再确认 `node.py` 通过 `load_prompt()` 加载。
