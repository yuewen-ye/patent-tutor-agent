...
feedback -> expert_a final review -> __end__
```
...
这条路径承担完整教学闭环：诊断画像、规划路径、检索材料、双专家生成与修订、裁判审核、反馈问卷、专家 A 最终审核答案。
...
- `teach` 正常接受路径闭环完整：`diagnosis -> planner -> tool_agent -> experts -> judge -> feedback -> expert_a final review` 的每一步都有上游状态输入和下游消费方。
...
- 最终答案不再由独立汇总节点生成；Judge 通过后交给专家 A 做最终审核并写入 `final_answer`。
