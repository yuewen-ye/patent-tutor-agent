...
本轮表现显示基础知识已非常稳固，且申请流程相关内容出现明确正答证据；当前更需要区分的是‘会做当前题型’还是‘能在新案例中稳定迁移’，因此后续应优先验证应用边界而非继续重复基础记忆。

## questionnaire

```json
[
  "请用自己的话说明：在专利申请审查中，‘基础规则理解’和‘具体流程判断’分别指什么？",
  "如果把同一知识点换成一个材料技术案例来考你，你觉得最容易出错的是哪一步：识别规则、选择规则，还是把规则落到案例上？"
]
```
...
```json
{
  "questions": [
    ...
    "从‘基础规则’过渡到‘申请流程’时，衔接是否清晰？",
    "你更希望后续多用文字梳理，还是多用案例对比来帮助确认边界？"
  ],
  "evaluation_signals": [
    "本轮三题均答对，且覆盖基础与流程两个相邻技能点",
    "当前没有明显错误暴露，难以仅凭结果判断教学表达是否需要调整"
  ],
  "feeds": "用于判断当前讲解节奏是否已足够支撑从基础到流程的平滑过渡，并帮助区分是知识稳定还是题型适配更优"
}
```
...
```json
{
  ...
  "cognition": {
    ...
    "evaluate": 0.31,
    "create": 0.12,
    "method": "结合本轮连续正确、既有理工与研发背景、以及基础与流程技能点的正向证据进行保守上调；由于缺少反例，应用以上层能力仍保持谨慎估计"
  },
  "style": {
    "perception": {
      "chosen": "sensing",
...
```json
{
  ...
  "plan_id": "3e8f41d3e57645fa9b0670ed8db72e05",
  "plan_version": 2
}
```
