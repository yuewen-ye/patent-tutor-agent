...
_prepare_integration ------------------> expert_a integration -> judge -> __end__
```
...
这条路径承担完整教学闭环：诊断画像、规划路径、检索材料、双专家辩论、专家 A 整合、Judge 最终裁决。通过后的 `expert_a_draft`（`draft_stage="integration"`）就是 teach 路由最终教学内容。
...
- `teach` 正常接受路径闭环完整：`diagnosis -> planner -> tool_agent -> experts debate -> expert_a integration -> judge` 的每一步都有上游状态输入和下游消费方。
