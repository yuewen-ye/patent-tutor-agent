...
- 决策：`keep`
- 决策原因：当前仅提供静态知识结构与易混淆对，没有学习者画像、BKT、当前活动计划或后端候选路线，无法可靠判断是否需要替换候选路线；在缺少输入的情况下保持路线是保守且可校验的选择。
...
- 类型：无
- 触发：若答题中出现对现有技术/抵触申请、三性判断、复审与无效程序的混淆，或对先修节点依赖表现不稳。
- 动作：继续按既定教学节奏推进，优先观察本轮对核心概念与易混淆点的掌握情况，若出现连续失配再收缩到前置节点复习。
