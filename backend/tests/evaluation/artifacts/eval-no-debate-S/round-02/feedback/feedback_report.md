...
```json
[
  "如果把软件著作权、专利和商业秘密放在同一案例里区分，你会先看哪一个事实特征？",
  "当题目从基础概念切到相关法域时，你更容易出错在规则记忆、边界识别还是案例套用？"
]
```
...
```json
{
  "questions": [
    ...
    "如果后续加入一个贴近研发场景的案例，你更希望先给规则总览还是直接做案例拆解？"
  ],
  "evaluation_signals": [
    "本轮没有暴露明显卡顿，但也没有提供额外过程性反馈",
    "题目整体更像验证性检查，尚不足以判断讲解节奏对高阶迁移是否合适"
  ],
  "feeds": "这些信号主要用于下一轮判断是否需要增加对比案例密度、降低切换速度，或把更多精力放在边界辨析而不是基础回忆上。"
}
```
