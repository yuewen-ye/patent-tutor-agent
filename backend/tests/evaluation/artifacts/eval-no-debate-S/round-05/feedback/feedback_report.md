...
```json
[
  "请用一句话区分“专利权利边界判断”和“专利可专利性判断”，并说明它们在案例分析中的先后顺序。",
  "当一个算法实现同时涉及开源组件和独立研发时，你会先核查哪类事实来判断专利风险？",
  "请说明软著与专利在保护对象上最核心的差别是什么，以及这种差别会如何影响你的结论。"
]
```
...
```json
{
  "questions": [
    ...
    "用案例推进边界判断时，例子是否足够具体、足够帮助你做出选择？",
    "当前题目难度更接近“确认已会”还是“开始有挑战但可跟上”？"
  ],
  "evaluation_signals": [
    "本轮未见明显答题犹豫",
    "基础确认题表现稳定",
    "当前更适合转入案例型迁移而非继续重复定义题"
  ],
  "feeds": "这些信号支持把下一轮难度从基础确认提升到案例辨析，重点观察学习者是否能把稳定的规则知识迁移到具体事实判断中。"
}
```
