...
```json
[
  "请不用原题表述，说明专利法基础在专利制度中的位置，以及它和后续申请、审查模块的关系。",
  "遇到一个新案例时，你会先看事实中的哪三个要素来判断它是否落入专利法基础框架？"
]
```
...
```json
{
  "questions": [
    "本轮题目节奏是否偏快、适中还是偏慢？",
    "法条原文式表述对你理解基础概念是否有帮助？",
    "后续如果加入短案例，你更希望先看提示再作答，还是直接独立判断？"
  ],
  "evaluation_signals": [
    "本轮作答无异常停顿，答题节奏稳定",
    "基础题连续正确，说明当前节奏与难度匹配度较高"
  ],
  "feeds": "这些信号支持继续保持顺序化、术语精确的讲解方式，并把下一轮重点放在案例迁移而不是基础概念重复确认。"
}
```
