...
```json
[
  "请用自己的话区分“专利基础规则已掌握”和“能把规则迁移到新案例”这两种状态。",
  "如果把题目换成智能制造场景，你最需要先确认的是事实识别、规则选择，还是例外条件？",
  "请说明一条你认为最容易与当前知识点混淆的专利规则，并说出它们的关键区别。"
]
```
...
```json
{
  "questions": [
    ...
    "题干中的表述是否足以让你区分考点，还是还需要更明确的事实线索？",
    "如果下一轮继续用案例题，你希望案例信息更简洁还是更完整？"
  ],
  "evaluation_signals": [
    "本轮没有出现明显停顿或反复修改答案的行为",
    "答题结果显示基础题段表现稳定"
  ],
  "feeds": "可据此保持较快节奏进入相邻主题，但后续仍需用案例化题目检验迁移能力，避免仅在基础题上形成过高判断。"
}
```
...
```json
{
  ...
  "plan_id": "11b7b2c090d54ccdab7536ade018309e",
  "plan_version": 2
}
```
