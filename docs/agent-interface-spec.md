...
| 领域专家 A | `expert_a` | 保守严谨、法条优先：生成→审查B→修订→联合合成 | `expert_a_draft`、`cross_review_a`、`revision_record_a` | `EXPERT_A_PROVIDER` | ✅（仅生成阶段；审查/修订/联合合成 [P0.1]） |
| 领域专家 B | `expert_b` | 生动灵活、面向案例：生成→审查A→修订→联合合成 | `expert_b_draft`、`cross_review_b`、`revision_record_b` | `EXPERT_B_PROVIDER` | ✅（仅生成阶段；审查/修订/联合合成 [P0.1]） |
| 审核裁判 Agent | `judge` | 审核联合合成稿（当前审两份独立草稿），只评估和提出修订建议 | `judge_report` | `JUDGE_PROVIDER` | ✅（当前审独立草稿；审联合合成稿 [P0.1]） |
...
| 汇总节点 | `finalize` | LLM 合并专家草稿/联合合成稿为最终答案 | `final_answer` | — | ✅ |
| 联合合成节点 | `joint_synthesis` | 专家 A+B 协作整合为一份输出 [P0.1] | `joint_synthesis_output` | — | [P0.1] |
| 轻量互审节点 | `lightweight_review` | Judge 打回后只审变更段落±1 [P0.1] | `lightweight_review_result` | — | [P0.1] |
...
| `max_debate_rounds` | integer | 最大辩论轮次，当前默认 2；P0.1 升级为默认 3 |
...
| 字段 | 类型 | 必填 | 写入方 | 读取方 | 来源 |
| --- | --- | --- | --- | --- | --- |
| `learner_profile_v2` | `LearnerProfileV2` | 否 | `diagnosis` | `planner`、`expert_b`、`feedback` | P0.3 |
| `learning_path_candidates` | array[`PathCandidate`] | 否 | `pathfinder` | 系统编排器、前端 | P0.2 |
| `cross_review_a` | `CrossReview` | 否 | `expert_a` | `expert_b` | P0.1 |
| `cross_review_b` | `CrossReview` | 否 | `expert_b` | `expert_a` | P0.1 |
| `revision_record_a` | `RevisionRecord` | 否 | `expert_a` | `judge` | P0.1 |
| `revision_record_b` | `RevisionRecord` | 否 | `expert_b` | `judge` | P0.1 |
...
| `profile_delta` | `ProfileDelta` | 否 | `feedback` | `pathfinder` | P0.3 |
| `bkt_states` | array[`BKTState`] | 否 | `feedback` | `diagnosis`、`pathfinder` | P0.4 |
| `recommend_reroute` | boolean | 否 | `feedback` | 条件路由 | P0.6 |
...
#### 6.4.2 阶段二：交叉审查 CrossReview [P0.1 待实现]
...
#### 6.4.3 阶段三：收到审查后修订 RevisionRecord [P0.1 待实现]
...
#### 6.4.4 阶段四：联合合成 JointSynthesis [P0.1 待实现]
...
| Judge 审核维度 | accuracy_score + adaptation_score | 三维度：事实准确性 + 完整性(depth对照) + 适配性(画像对照) | P0.1 |
