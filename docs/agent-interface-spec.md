...
- teach 路径保留完整诊断→规划→检索→双专家循环→Judge→反馈→专家 A 最终审核流程
...
| 领域专家 A | `expert_a` | 保守严谨、法条优先：生成草稿、按 Judge 要求修订、在通过后做最终审核输出 | `expert_a_draft`、`final_answer` | `EXPERT_A_PROVIDER` | ✅ |
...
| `expert_a_draft` | `ExpertDraft` | 否 | `expert_a` | `judge`、`expert_a` 最终审核 |
| `expert_b_draft` | `ExpertDraft` | 否 | `expert_b` | `judge`、`expert_a` 最终审核 |
| `judge_report` | `JudgeReport` | 否 | `judge` | `feedback`、修订路由、`expert_a` 最终审核 |
| `feedback_result` | `FeedbackResult` | 否 | `feedback` | `expert_a` 最终审核 |
| `final_answer` | `FinalAnswer` | 否 | `expert_a` | API / 前端 |
...
`kind` 允许值：`learner_profile_report`、`learning_path_plan`、`retrieval_context`、`expert_draft`、`judge_report`、`feedback_report`、`final_answer`、`route_decision`、`chat_answer`。
...
| 值 | 含义 | 路由 |
| --- | --- | --- |
| `accept` | 可直接进入反馈和专家 A 最终审核 | `feedback` |
| `accept_with_minor_revision` | 可进入反馈和专家 A 最终审核 | `feedback` |
...
轮次上限默认 3。达到上限后，即使仍为 `revise`，也进入 `feedback` 和专家 A 最终审核。

### 7.5 反馈后最终审核 ✅ 当前实现

```text
feedback -> expert_a final review -> END
```
...
feedback -> expert_a final review   when recommend_reroute=false
...
| 模块 | 当前 | P0 目标 | 涉及子任务 |
|------|------|---------|-----------|
| 专家工作流 | 双专家生成 → Judge 审核 → 定向修订循环 → 专家 A 最终审核 | 双专家承担更多职责，避免额外 LLM 节点 | 当前实现 |
| 专家互审 | 由 Judge 统一审核并提出定向修订请求 | 可按需求再设计为专家内部 prompt 职责 | 当前实现 |
| 最终答案 | 专家 A 最终审核输出 | 不再使用独立 finalize 节点 | 当前实现 |
