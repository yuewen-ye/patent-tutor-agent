...
- 模型 provider/model/temperature/top_k 不在 Agent 节点中写死，运行时优先由 `config/agents.yaml` 和 `AgentLLMRouter` 决定；温度字段只能用于支持该参数的模型，GPT-5.6 配置温度会在加载阶段失败且请求层不会发送该字段；`.env` 只放 API key 和本机路径，旧 `*_PROVIDER` 等环境变量仅作兼容回退。
