...
- teach 路径保留完整诊断→规划→检索→双专家辩论循环→Judge→专家 A 整合→Judge 终审流程
...
| 领域专家 A | `expert_a` | 保守严谨、法条优先：生成草稿、按 Judge 要求修订、在 A/B 辩论完成后整合两方结果 | `expert_a_draft` | `EXPERT_A_PROVIDER` | ✅ |
| 领域专家 B | `expert_b` | 生动灵活、面向案例：生成草稿、按 Judge 要求修订 | `expert_b_draft` | `EXPERT_B_PROVIDER` | ✅ |
| 审核裁判 Agent | `judge` | 直接审核专家 A/B 草稿（三维度评分），只评估和提出修订建议 | `judge_report` | `JUDGE_PROVIDER` | ✅ |
| 学情诊断 Agent（反馈阶段） | `feedback` | 生成问卷、下一步动作、画像变化向量 Δ | `feedback_result`、`profile_delta` | `DIAGNOSIS_PROVIDER` | ✅（当前无 Δ 输出 [P0.3]） |
...
| `retrieval_context` | array[`RetrievalChunk`] | 否 | `tool_agent` | `expert_a`、`expert_b`、`judge`、`chat_answer` |
| `tool_agent_answer` | string | 否 | `tool_agent` | `chat_answer` |
| `expert_a_draft` | `ExpertDraft` | 否 | `expert_a` | `judge`、专家 A 整合阶段、API / 前端 |
| `expert_b_draft` | `ExpertDraft` | 否 | `expert_b` | `judge`、专家 A 整合阶段 |
| `judge_report` | `JudgeReport` | 否 | `judge` | 修订路由、专家 A 整合阶段 |
...
`created_by` 允许值：`diagnosis`、`planner`、`expert_a`、`expert_b`、`judge`、`feedback`、`route`、`tool_agent`、`chat_answer`、`revise_experts`。
...
| 值 | 含义 | 路由 |
| --- | --- | --- |
| `accept` | debate 阶段进入专家 A 整合；integration 阶段结束 | `expert_a` 或 `END` |
| `accept_with_minor_revision` | debate 阶段进入专家 A 整合；integration 阶段结束 | `expert_a` 或 `END` |
| `revise` | 需要专家按裁判建议重写 | `revise_experts`，若达到轮次上限则进入专家 A 整合或结束 |

Judge 不得写教学正文，只能写争议、裁决、理由和修订请求。节点内置 `_normalize_judge_report()` 和 `_normalize_target()` 处理 LLM 输出的非标准值。

Judge 直接审核两份专家草稿。审核维度包括：
...
读取：`expert_a_draft`、`expert_b_draft`、`judge_report`、`retrieval_context`。
...
3. 必须回应 Judge 已通过、轻微修订或达到轮次上限时留下的裁判摘要。
4. 输出连贯的统一教学文本，非简单拼接。
...
```text
judge -> expert_a integration        when phase=debate and decision in accept|accept_with_minor_revision
judge -> revise_experts              when phase=debate and decision=revise and debate_round < max_debate_rounds
judge -> expert_a integration        when phase=debate and decision=revise and debate_round >= max_debate_rounds
judge -> END                         when phase=integration and decision in accept|accept_with_minor_revision
judge -> revise_experts              when phase=integration and decision=revise and debate_round < max_debate_rounds
judge -> END                         when phase=integration and decision=revise and debate_round >= max_debate_rounds
revise_experts -> expert_a ∥ expert_b  parallel revision during debate phase
revise_experts -> expert_a             integration revision during integration phase
...
轮次上限默认 3。debate 阶段达到上限后，即使仍为 `revise`，也进入专家 A 整合；integration 阶段达到上限后结束，保留当前整合稿和 Judge 报告。

### 7.5 专家 A 整合后终审 ✅ 当前实现

```text
expert_a integration -> judge -> END
```
...
| 模块 | 当前 | P0 目标 | 涉及子任务 |
|------|------|---------|-----------|
| 专家工作流 | 双专家生成 → Judge 审核 → 定向修订循环 → 专家 A 整合 → Judge 终审 | 双专家承担更多职责，避免额外 LLM 节点 | 当前实现 |
| 专家互审 | 由 Judge 统一审核并提出定向修订请求 | 可按需求再设计为专家内部 prompt 职责 | 当前实现 |
