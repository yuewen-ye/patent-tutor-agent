...
4. **模式判断**：根据输入是否包含 `revision_requests` 判断当前模式，两种模式的生成策略完全不同：
   ...
   - **修订模式**（有 `revision_requests`）：以双专家修改稿为权威源、上一轮整合稿为基准，**针对全部累积必须修改项（含历史轮次）逐条定点修订对应内容点**。可重写被意见涉及的内容段落，但**不重排结构、不把结构化模块搬进 `teaching_content`**；未被提及的部分必须保持原样不动，`block_plan` 结构不得变更，仅修改涉及 block 的 `payload`。
5. **全局修正传染性（穿透力兜底）**：当某条 `revision_request` 涉及法条含义、概念定义、时间基准或事实主张类全局修正时，必须同步修正到 `teaching_content` 与所有相关 block payload，并删除矛盾原文；不得只在被点名处改，避免同一错误在多处残留。
...
- `assessment.items` 题目同样须为**客观题**：必须有唯一确定答案，不得为开放作答或自由论述；如"如何评估XX风险"类问法必须改写成有确定结论的客观表述，禁止留为开放式。
