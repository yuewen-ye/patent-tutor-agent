...
def test_call_llm_fails_over_to_candidate_provider(monkeypatch) -> None:
    monkeypatch.setattr("backend.app.core.llm.load_dotenv", lambda *a, **k: None)
    monkeypatch.setattr(
        "backend.app.core.llm.provider_runtime_config",
        lambda _provider: ProviderRuntimeConfig(),
    )
    monkeypatch.setenv("LLM_FAILOVER_PROVIDERS", "qwen")
    monkeypatch.setenv("LLM_RETRY_TIMES", "1")
    monkeypatch.setenv("DEEPSEEK_API_KEY", "deepseek-key")
    monkeypatch.setenv("DEEPSEEK_BASE_URL", "https://primary.invalid/v1")
    monkeypatch.setenv("QWEN_API_KEY", "qwen-key")
    monkeypatch.setenv("QWEN_BASE_URL", "https://candidate.invalid/v1")

    def handler(request: httpx.Request) -> httpx.Response:
        if "primary.invalid" in str(request.url):
            return httpx.Response(500, json={"error": {"message": "upstream down"}})
        return _json_response("failover-ok")
...
def test_call_llm_failover_skips_unconfigured_candidates(monkeypatch) -> None:
    monkeypatch.setattr("backend.app.core.llm.load_dotenv", lambda *a, **k: None)
    monkeypatch.setattr(
        "backend.app.core.llm.provider_runtime_config",
        lambda _provider: ProviderRuntimeConfig(),
    ...
    client = httpx.Client(
        transport=httpx.MockTransport(
            lambda request: httpx.Response(500, json={"error": {"message": "primary down"}})
        )
    )
...
def test_call_llm_failover_disabled_via_env(monkeypatch) -> None:
    monkeypatch.setattr("backend.app.core.llm.load_dotenv", lambda *a, **k: None)
    monkeypatch.setattr(
        "backend.app.core.llm.provider_runtime_config",
        lambda _provider: ProviderRuntimeConfig(),
    )
    monkeypatch.setenv("LLM_FAILOVER_PROVIDERS", "none")
    monkeypatch.setenv("LLM_RETRY_TIMES", "1")
    monkeypatch.setenv("QWEN_API_KEY", "qwen-key")
    monkeypatch.setenv("DEEPSEEK_API_KEY", "deepseek-key")
    monkeypatch.setenv("DEEPSEEK_BASE_URL", "https://primary.invalid/v1")
    ...
    client = httpx.Client(
        transport=httpx.MockTransport(
            lambda request: httpx.Response(500, json={"error": {"message": "all down"}})
        )
    )

    with pytest.raises(LLMProviderError, match="all down"):
        call_llm(
            provider="deepseek",
            messages=[LLMMessage(role="user", content="x")],
            http_client=client,
