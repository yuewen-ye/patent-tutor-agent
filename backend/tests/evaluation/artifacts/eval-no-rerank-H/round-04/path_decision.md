...
- 决策：`keep`
- 决策原因：当前仅提供了静态知识图和易混淆对，没有学习者画像、BKT掌握度、当前活动计划或后端候选路线，因此无法判断是否应保留或替换候选路线；在缺少候选路线的前提下，采用保守策略，先不给出节点重排。
...
- 类型：无
- 触发：缺少后端算法候选路线、学习者画像和BKT信息，无法进行有效规划决策
- 动作：等待补充候选路线与学情输入后再执行路线调整
