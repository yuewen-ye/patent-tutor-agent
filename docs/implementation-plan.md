...
| 模块 | 状态 | 说明 |
|---|---|---|
| LangGraph 编排层 | 🟡 基础完成 | 三路由 workflow + 辩论闭环；专家协作链（交叉审查→联合合成→轻量互审）待实现 |
| StateDict 合同 | ✅ 完成 | Pydantic 模型 + JSON Schema 导出 |
| LLM Provider 路由 | ✅ 完成 | 三层架构，10 Agent 各自独立路由 |
| Artifact 产物落盘 | ✅ 完成 | `_with_artifacts` 包装 + manifest.json |
| CLI Demo | ✅ 完成 | `run_workflow.py` + `show_workflow.py` |
| 测试 | ✅ 完成 | 覆盖 workflow/LLM/contracts/记忆系统/三路由 |
...
| 工作流完善 | ❌ 待实现 | 专家交叉审查、联合合成、知识图谱 A* 搜索、五维画像、BKT |
...
LLM 生成学习路径                     A* 算法在知识图谱上搜索路径
简化画像（5 字段）                   五维画像（knowledge/cognition/style/progress/affect）
无 BKT                               BKT 贝叶斯知识追踪（4 参数模型）
planner 独立调用 RAG                 每个 Agent 独立调用 RAG（各自检索不同内容）
```
...
| # | 任务 | 涉及文件 | 说明 |
|---|---|---|---|
| P0.1.1 | 扩展 `expert_a` 节点：交叉审查模式 | `backend/app/agents/expert_a/node.py` | 新增审查 prompt（四类别标记体系），接收对方的草稿作为输入，输出审查意见 |
| P0.1.2 | 扩展 `expert_b` 节点：交叉审查模式 | `backend/app/agents/expert_b/node.py` | 新增审查 prompt（四类别标记体系），输出含改写方案的审查意见 |
| P0.1.3 | 扩展 `expert_a` 节点：修订模式 | `backend/app/agents/expert_a/node.py` | 接收对方的审查意见，逐条回应，输出修订稿+修订记录 |
| P0.1.4 | 扩展 `expert_b` 节点：修订模式 | `backend/app/agents/expert_b/node.py` | 同上 |
| P0.1.5 | 新增联合合成节点 | `backend/app/agents/joint_synthesis.py`（新建） | 或作为 expert_a/b 的阶段四，接收双方修订稿，协作整合 |
| P0.1.6 | 新增轻量互审节点 | `backend/app/agents/lightweight_review.py`（新建） | Judge 打回后，双专家只审变更段落+前后各一段 |
| P0.1.7 | 重构工作流图 | `backend/app/graph/workflow.py` | 将五阶段协作链接入 teach 路径 |
| P0.1.8 | 扩展 StateDict | `backend/app/schemas/state.py` | 新增 `cross_review_a`、`cross_review_b`、`revision_record_a`、`revision_record_b`、`joint_synthesis` 等字段 |
| P0.1.9 | 扩展 Judge 输入 | `backend/app/agents/judge/node.py` | Judge 改为审核联合合成稿（而非两份独立草稿） |
| P0.1.10 | 扩展 max_debate_rounds | `backend/app/graph/workflow.py` | 从默认 2 轮改为 3 轮 |
...
```
当前 StateDict 字段           P0 后新增
─────────────────────        ──────────────────────────
learner_profile              learner_profile_v2（五维）
learning_path                learning_path_candidates（3条候选）
expert_a_draft               cross_review_a_by_b
expert_b_draft               cross_review_b_by_a
judge_report                 revision_record_a
feedback_result              revision_record_b
（无）                       joint_synthesis_output
（无）                       lightweight_review_result
（无）                       profile_delta（变化向量 Δ）
（无）                       recommend_reroute
（无）                       bkt_states
```
