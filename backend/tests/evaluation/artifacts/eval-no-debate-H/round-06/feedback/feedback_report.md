...
```json
[
  "请说明材料领域专利申请中，哪些信息最能帮助你判断是新颖性问题还是创造性问题？",
  "如果把同一技术方案放到一个新的对比案例里，你会先检查哪个判断步骤，为什么？"
]
```
...
```json
{
  "questions": [
    ...
    "题干和选项的表述是否足够清晰，没有造成额外歧义？",
    "你觉得当前案例化呈现是否比纯定义回忆更容易定位规则边界？"
  ],
  "evaluation_signals": [
    "本轮三题全部判定正确，说明当前基础讲解与练习难度没有超出可处理范围",
    "学习目标仍集中在专利审查规则的概念边界与案例判断"
  ],
  "feeds": "用于后续节奏和呈现方式的适配判断，并辅助区分是内容边界问题还是表达负荷问题"
}
```
