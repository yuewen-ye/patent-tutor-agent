...
BLOCK_CONTENT_SPEC: dict[str, dict[str, Any]] = {
    ...
    "worked_example": {
        ...
        "fields": [
            ...
            {"key": "steps", "desc": "分步推演列表，每步含 reasoning(推理) 与 summary(小结)，展示判定链如何走", "min": "≥3 步"},
        ...
        "example": {
            ...
            "steps": [
                {"reasoning": "展出日在申请日前，且属‘政府主办国际展会’法定情形", "summary": "落入宽限期适用范围"},
                {"reasoning": "申请日 2023-10-20 距展出日 2023-05-01 约 5.5 个月 < 6 个月", "summary": "在宽限期内"},
                {"reasoning": "宽限期仅豁免该公开，不改变三性实质审查", "summary": "仍需过创造性/实用性"},
            ],
    ...
    "decision_flow": {
        ...
        "fields": [
            ...
            {"key": "steps", "desc": "决策步骤列表，每步含 condition(条件) 与 outcome(走向)，形成可走的分支", "min": "≥3 步"},
            {"key": "end_states", "desc": "各分支的终态/结论列表", "min": "非空"},
        ],
        "example": {
            "question": "一个公开行为是否破坏新颖性？",
            "steps": [
                {"condition": "公开日在申请日之后", "outcome": "不可能破坏（尚未公开）"},
                {"condition": "公开日在申请日前且属宽限期情形且在 6 个月内申请", "outcome": "不破坏"},
                {"condition": "公开日在申请日前且不属于宽限期/超期", "outcome": "构成现有技术，破坏新颖性"},
            ],
    ...
    "verbal_explanation": {
        ...
        "fields": [
            ...
            {"key": "key_terms", "desc": "本段点破的术语列表（每项含 term(术语) 与 explanation(一句话人话解释)）", "min": "非空"},
        ...
        "example": {
            ...
            "key_terms": [{"term": "新颖性", "explanation": "申请前没被公开过"}, {"term": "宽限期", "explanation": "特定公开给的 6 个月缓冲"}],
        },
    },
    "predict_activate": {
    ...
    "mnemonic": {
        ...
        "fields": [
            ...
            {"key": "mapping", "desc": "device 中每个元素对应什么知识点（列表，每项含 term(元素) 与 explanation(对应知识点)）", "min": "非空"},
            {"key": "when_recall", "desc": "何时使用此记忆锚（触发场景）", "min": "非空"},
        ],
        "example": {
            "device": "三性记忆表：新(没公开过) / 创(不显而易见) / 实(能做有用)",
            "mapping": [{"term": "新", "explanation": "新颖性=未公开"}, {"term": "创", "explanation": "创造性=非显而易见"}, {"term": "实", "explanation": "实用性=可制造有用"}],
    ...
    "summary_card": {
        ...
        "fields": [
            {"key": "cards", "desc": "要点卡列表，每张含 concept(概念) 与 one_liner(一句话)，覆盖本节点核心", "min": "≥3 张"},
            {"key": "must_recite", "desc": "必背条目列表（考试/实务硬通货）", "min": "非空"},
            {"key": "one_line", "desc": "本节点一句话总结", "min": "非空"},
        ],
        "example": {
            "cards": [
                {"concept": "保护客体", "one_liner": "能申请专利的‘东西’范围"},
                {"concept": "三性", "one_liner": "新颖性/创造性/实用性三关"},
                {"concept": "宽限期", "one_liner": "特定公开给 6 个月缓冲"},
            ],
