...
> 本提示词用于评估每轮课程的质量指标，包含以下维度（2026-09-03 调整：删除旧「1.2 幻觉评估 \[LLM]」独立维度；原 1.3/1.4/1.5 系列前移一位）：
>
> **整体评估维度（一次调用）**：
>
> - ~~1.2 幻觉评估~~（已并入 1.2.x 三分类谬误率 + 1.2 谬误率汇总，见 statement 模式）
>
...
> **陈述级评估维度（一次调用）**：
>
> - **1.2.1 事实性谬误率**：事实性错误陈述比例
>
> - **1.2.2 逻辑性谬误率**：逻辑性错误陈述比例
>
> - **1.2.3 指令性谬误率**：指令性错误陈述比例
>
> - **1.3.1 知识溯源可验证率**：来源真实存在且可验证的比例
>
> - **1.3.2 溯源内容支撑率**：来源内容支撑核心断言的比例
>
> **检索评估维度（一次调用）**：
>
> - **4.1 检索准确率**：检索到的 chunk 与知识库/权威事实一致的比例
>
> - **4.2 检索完整率**：检索到的 chunk 覆盖回答问题所需要点的比例
>
> **覆盖率评估维度（一次调用）**：
>
...
- **目标覆盖度 (goal\_coverage)**：是否覆盖了学习路径中的学习目标

- **事实/法律准确性 (factual\_accuracy)**：法条引用、案例描述是否准确

- **案例准确性 (case\_accuracy)**：案例描述是否真实、事实是否正确

- **事实一致性 (factual\_consistency)**：课程内部陈述是否前后一致

- **教学清晰度 (pedagogical\_clarity)**：讲解逻辑是否通顺、是否易懂

- **难度适配性 (difficulty\_fit)**：题目难度是否符合学习路径要求

- **学员匹配度 (learner\_fit)**：课程是否考虑学员的薄弱点和学习风格

- **知识完整性 (knowledge\_completeness)**：知识点覆盖是否完整

- **薄弱点针对性 (weakness\_addressing)**：是否针对性解决学员薄弱点

## 整体评估输出格式

> 2026-09-03：**hallucination 字段已从核心指标删除**（下次整体评估不再输出该块）。
> 幻觉相关统一由 `statement` 模式产出 1.2.1/1.2.2/1.2.3 三分类谬误率 + 1.2 谬误率汇总。
...
1. **M1.2 三类谬误判定**（事实性 / 逻辑性 / 指令性）
2. **M1.3 溯源可靠性**（可验证率 / 支撑率）

## 评估标准

### 1. 正确性评估（对应 M1.2 三类谬误（原 M1.4））
...
### 2. 溯源可靠性评估（对应 M1.3 知识溯源（原 M1.5））

#### 2.1 知识溯源可验证率（source\_verifiability）

判断陈述所引用的来源（法条号、文件出处）是否真实存在。

- **90-100分 (verified)**：来源真实存在，法条号准确无误

- **70-89分 (partially\_verified)**：来源真实存在，法条号有极少量偏差

- **50-69分 (partially\_verified)**：来源存在但法条号有明显错误

- **30-49分 (unverified)**：来源存在但无法具体对应

- **0-29分 (unverified)**：来源不存在或完全虚构

#### 2.2 溯源内容支撑率（content\_relevance）

判断陈述所引用来源的内容是否直接支撑陈述的核心断言。

- **90-100分 (relevant)**：来源内容完全支撑陈述

- **70-89分 (partially\_relevant)**：来源内容基本支撑，有少量偏差

- **50-69分 (partially\_relevant)**：来源内容部分支撑
...
- **50-69分 (partially\_accurate)**：部分内容准确，存在少量与权威不符的内容
...
- **50-69分 (partially\_complete)**：覆盖了主要要点，但有明显遗漏
...
### 1. 本节知识点覆盖率 (section\_coverage) — M3.1
...
### 2. 薄弱点命中率 (weakness\_coverage) — M3.2
...
### 3. 混淆对覆盖率 (confusion\_coverage) — M3.3
