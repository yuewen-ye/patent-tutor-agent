-- Fresh-install schema. This project deliberately does not migrate prior schemas.
...
CREATE TABLE IF NOT EXISTS learner_learning_plans (
  ...
  updated_at DATETIME(6) NOT NULL,
  completed_at DATETIME(6),
  CONSTRAINT fk_learner_plans_student FOREIGN KEY(student_id) REFERENCES students(student_id),
  CONSTRAINT fk_learner_plans_source_session FOREIGN KEY(source_session_id) REFERENCES sessions(session_id),
  CONSTRAINT fk_learner_plans_last_session FOREIGN KEY(last_session_id) REFERENCES sessions(session_id),
  UNIQUE KEY uq_learner_plan_version(student_id, plan_version),
  CHECK(status IN ('active', 'completed', 'superseded'))
) ENGINE=InnoDB;
CREATE INDEX ix_learner_plans_active ON learner_learning_plans(student_id, status, plan_version);
...
CREATE TABLE IF NOT EXISTS learner_learning_plan_nodes (
  ...
  created_at DATETIME(6) NOT NULL,
  updated_at DATETIME(6) NOT NULL,
  CONSTRAINT fk_learner_plan_nodes_plan FOREIGN KEY(plan_id) REFERENCES learner_learning_plans(plan_id),
...
CREATE TABLE IF NOT EXISTS questions (
  ...
  session_id VARCHAR(128) NOT NULL,
  round_id VARCHAR(128),
  origin VARCHAR(32) NOT NULL DEFAULT 'generated_course',
  ...
  CHECK(origin IN ('generated_course', 'diagnostic_catalog')),
  CHECK(kind IN ('interactive', 'assessment', 'diagnostic')),
...
CREATE TABLE IF NOT EXISTS attempts (
  ...
  UNIQUE KEY uq_attempt_student_idempotency(student_id, idempotency_key),
...
CREATE TABLE IF NOT EXISTS mastery_events (
  ...
  event_kind VARCHAR(32) NOT NULL DEFAULT 'observed',
  ...
  created_at DATETIME(6) NOT NULL,
  CONSTRAINT fk_mastery_events_student FOREIGN KEY(student_id) REFERENCES students(student_id),
  CONSTRAINT fk_mastery_events_attempt FOREIGN KEY(attempt_id) REFERENCES attempts(attempt_id),
  UNIQUE KEY uq_mastery_event_attempt_node_kind(attempt_id, node_id, event_kind),
  KEY ix_mastery_events_student_node_time(student_id, node_id, created_at),
  CHECK(event_kind IN ('observed', 'inferred')),
