...
```json
[
  "请用一句话区分专利基础规则与相关法律规则在答题时各自更容易考查的能力。",
  "如果下一题换成情境判断，你会先抓规则边界还是先找适用条件？"
]
```
...
```json
{
  "questions": [
    "本轮题目节奏是否偏快、合适还是偏慢？",
    "基础复核题的表述是否足以区分你对规则边界的把握？",
    "如果加入一个短案例，你会更容易判断还是更容易混淆？"
  ],
  "evaluation_signals": [
    "本轮无需额外纠错",
    "当前节奏对稳定知识核验是足够的"
  ],
  "feeds": "用于下一轮判断是否需要增加情境化难度，以及是否继续保持顺序化、文字化呈现"
}
```
