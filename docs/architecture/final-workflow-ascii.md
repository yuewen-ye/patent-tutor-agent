...
`tool_agent` 不属于当前定义的核心 Agent 集合，主流程不再让它自主判断是否调用工具。RAG 检索由确定性的 `retrieve_context` workflow 节点完成；LLM Agent 读取 `retrieval_context`，但不在主流程中发起 tool-calling。
...
```text
route → diagnosis → planner → retrieve_context → expert_a/expert_b
      → revise_experts 循环
      → expert_a integration → judge → feedback → END
```

适用：课程页面的系统学习、学习路径规划。特点是完整诊断、规划、检索、专家辩论、专家 A 整合、Judge 审核、feedback 收尾。
...
| `retrieve_context` | 非 LLM workflow 节点 | 调用 `rag_retrieve()`，写入 `retrieval_context` |
| `expert_a` | LLM Agent | 法条优先、严谨草稿；最终负责整合 |
| `expert_b` | LLM Agent | 案例化、易懂草稿 |
